...
def _expand_onehot_labels(labels, label_weights, label_channels):
    bin_labels = paddle.full((labels.shape[0], label_channels), 0, dtype='float32')
    inds = paddle.nonzero(
...
def binary_cross_entropy(pred,
    ...
    Args:
        pred (paddle.Tensor): The prediction with shape (N, 1).
        label (paddle.Tensor): The learning label of the prediction.
        weight (paddle.Tensor, optional): Sample-wise loss weight.
        reduction (str, optional): The method used to reduce the loss.
    ...
    loss = F.binary_cross_entropy_with_logits(
        pred, label.astype('float32'), weight=class_weight, reduction='none')
...
def mask_cross_entropy(pred,
    ...
    Args:
        ...
        label (paddle.Tensor): ``label`` indicates the class label of the mask
            corresponding object. This will be used to select the mask in the
            class which the object belongs to when the mask prediction
            is not class-agnostic.
        reduction (str, optional): The method used to reduce the loss.
            Options are "none", "mean" and "sum".
        avg_factor (int, optional): Average factor that is used to average
            the loss. Defaults to None.
    ...
    inds = paddle.arange(0, num_rois, dtype='int64', device=pred.place)
