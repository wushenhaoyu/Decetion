...
from  my_seg.paddleseg.utils import get_image_list, logger
...
class Predictor:
    ...
    def run(self, img_list):
        ...
        for i in range(0, len(img_list), args.batch_size):
            # warm up
            if i == 0 and args.benchmark:
                for j in range(5):
                    data = np.array([ img
                        for img in img_list[0:args.batch_size]
                    ])
                    input_handle.reshape(data.shape)
                    input_handle.copy_from_cpu(data)
                    self.predictor.run()
            ...
            data = np.array(
                [self._preprocess(img) for img in img_list[i:i + args.batch_size]])
            input_handle.reshape(data.shape)
            input_handle.copy_from_cpu(data)
